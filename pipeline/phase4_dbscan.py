"""
pipeline/phase4_dbscan.py
=========================
Phase 4 — Hierarchy Construction  |  Clustering: DBSCAN

Algorithm
---------
L1 : Root label from Phase 3 degree-central root_node (LDA fallback).
L2 : DBSCAN on TF-IDF vectors of concept node labels.
     - eps=0.6, min_samples=1 (so no points are noise-labelled).
     - min_samples=1 means every point is a core point — DBSCAN here
       acts as a density-based grouper, not an outlier detector.
     - Noise points (label=-1) are absorbed into nearest cluster by
       cosine distance.
L3 : SPO-edge traversal from L2 representative + sibling fill-in (cap 8).

Key difference from K-Means/BIRCH
----------------------------------
DBSCAN does NOT require K to be specified upfront. It discovers the
number of clusters from the data density. On small concept sets (15-20
nodes) this often produces more or fewer clusters than K-Means would,
and may produce very unbalanced clusters — which is intentional when
some concepts are tightly related and others are isolated.
...
# ── DBSCAN clustering ─────────────────────────────────────────────────────────
def _dbscan_cluster(ids, vecs, eps=0.6, min_samples=1):
    """
    Run sklearn DBSCAN on TF-IDF vectors (cosine metric).
    eps         — neighbourhood radius; increase to merge more concepts
    min_samples — set to 1 so every point is reachable (no noise labels)
    Noise points (label=-1) are absorbed into the nearest cluster.
    ...
    n = len(ids)
    if n == 0: return {}
    if n == 1: return {ids[0]: 0}

    X      = np.array(vecs, dtype=float)
    model  = DBSCAN(eps=eps, min_samples=min_samples, metric="cosine")
    labels = model.fit_predict(X)

    # Absorb any noise points (label == -1) into the nearest cluster
    ...
    if -1 in unique_labels and len(unique_labels) > 1:
        ...
        for i, lbl in enumerate(labels):
            if lbl == -1:
                ...
                best_j = min(
                    (j for j in range(n) if labels[j] != -1),
                    key=lambda j: _d2(vecs[i], vecs[j])
                )
                labels[i] = labels[best_j]

    # If DBSCAN put everything into one cluster, split by WEF rank
    if len(set(labels)) == 1:
        chunk = max(1, n // 3)
        labels = [i // chunk for i in range(n)]
...
def _build_one(topic_id, concept_result, lda_result, depth):
    ...
    cmap  = _dbscan_cluster(sids, vecs, eps=0.6, min_samples=1)
...
def run(sentence_groups, concept_results, lda_result, depth=3, verbose=False):
    ...
    for tid in sorted(sentence_groups):
        ...
        if verbose:
            print(f"  [Phase4-DBSCAN] Topic {tid} root='{h['label']}': "
                  f"L2={len(h['children'])}  "
